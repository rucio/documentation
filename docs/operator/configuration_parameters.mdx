...
[client connection information](#client_config) or the
...
#### **cache**
  ...
  - **use_external_cache_for_auth_tokens**: _(Optional)_ if True, use remote
    ...
    Default: `False`
#### **cli**
  - **endpoints**: _(Optional)_ Explicitly set the commands accessed from the CLI. `ping`, `whoami` cannot be added or removed.
...
#### **client** {#client_config}
  ...
  - **username**: Mandatory if `auth_type = userpass`.
...
#### **repairer**
  - **updated_at_delta**: _(Optional)_ The minimum time in seconds since the last update of a rule
    for STUCK rules to be considered for repairing. Can be set to ``-1`` to ignore the ``updated_at``
    timestamp and always try repairing STUCK rules. Default: `1800`.
#### **test**
...
Mutable settings are `deterministic`, `rse_type`, `staging_area`, `volatile`, `availability_delete`, `availability_read`, `availability_write`, `city`, `country_name`, `latitude`, `longitude`, `region_code`, and `time_zone`.
...
- **rse_type**: String. Specify `DISK` or `TAPE`. Default: `DISK`.
...
- **volatile**: Boolean. Specifies if the RSE is cache storage. Default: `False`.

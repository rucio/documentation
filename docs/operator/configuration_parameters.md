...
## RSE settings
The RSE settings are set separately using `rucio.RSEClient.update_rse` or `rucio-admin rse update`, and specifies RSE configuration used by the Rucio instance.
...
- **credentials**: Dictonary[String, Any]: Specify credentials used for accessing this RSE if it is in a commercial cloud. No default.
- **delete_protocol**: Integer: Cannot be changed. Default: `1`.
...
- **lfn2pfn_algorithm**: String: Name of the algorithm in the configured policy package to be used for creating replica paths. Cannot be changed. Default: `default`.
- **qos_class**: String: No default.
- **read_protocol**: Integer: Cannot be changed. Default: `1`.
...
- **staging_area**: Boolean. Default: `False`.
- **third_party_copy_read_protocol**: Integer: Cannot be changed. Default `1`.
- **third_party_copy_write_protocol**: Integer: Cannot be changed. Default `1`.
...
- **write_protocol**: Integer: Cannot be changed. Default: `1`.
...
- **sign_url** Boolean. Controls if URLs for uploads and transfers are to be cryptographically signed by an external service. No default.
...
- **staging_buffer**: String. Used with `TAPE` RSEs to specify where a file on tape should be transferred to as a disk buffer. No default.

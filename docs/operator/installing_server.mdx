...
with configuration for RSEs as described [here](#defining-path-for-storage-capabilities)
...
#### Configuring `<path>` for Storage Capabilities in Rucio: {#defining-path-for-storage-capabilities}

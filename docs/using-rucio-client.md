...
`idpsecrets.json` file). In the following examples we assume
...
If `auth_type`[ is specified to be *"oidc"* in the *rucio.cfg*
file, ]`-S` can be omitted as well.
...
The state indicates how many locks (physical replicas of the files) are
